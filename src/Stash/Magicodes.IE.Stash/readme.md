...
### 从Excel文件导入:
 ...
 映射规则用于描述数据源中的每一列,如何映射到dao中的属性，在规则里可以通过 `转换器` 管道对数据流进行多次转换，最终输出为需要的值。

 `转换器` 是一段C#脚本，可以调用宿主项目中已定义的库和方法，为导入提供更多的灵活性。
...
- 变量定义： 可以在映射规则中定义变量，变量的值可以是常量，也可以是脚本计算的值，变量可以被规则中其它地方的脚本引用。
